    ...
    <div class="flex items-start justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">
                <a href="{{ url('/') }}" class="hover:underline">Inicio</a> / Proveedores
            </p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-gray-500 text-xs uppercase">
                <tr>
                    <th class="text-left px-5 py-3 font-medium">Proveedor</th>
                    <th class="text-left px-5 py-3 font-medium">Contacto</th>
                    <th class="text-left px-5 py-3 font-medium">RUC</th>
                ...
                @forelse ($proveedores as $proveedor)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-3 text-gray-600">
                            ...
                            <div class="text-xs text-gray-400">{{ $proveedor->telefono ?? '' }}</div>
                        </td>
                        <td class="px-5 py-3 text-gray-600">{{ $proveedor->ruc ?? '—' }}</td>
                        <td class="px-5 py-3 text-gray-600">{{ $proveedor->lentes_count }}</td>
                        <td class="px-5 py-3">
                            <span class="text-xs font-semibold px-2 py-1 rounded-full {{ $proveedor->activo ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $proveedor->activo ? 'Activo' : 'Inactivo' }}
                            </span>
                        </td>
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('proveedores.show', $proveedor) }}" class="p-2 rounded-lg border border-gray-200 hover:bg-gray-100" aria-label="Ver">
                                ...
                                <a href="{{ route('proveedores.edit', $proveedor) }}" class="p-2 rounded-lg border border-gray-200 hover:bg-gray-100" aria-label="Editar">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-5 py-10 text-center text-gray-400">
                            ...
                            <a href="{{ route('proveedores.create') }}" class="text-emerald-700 font-medium hover:underline">Crea el primero</a>.
                        </td>
                    </tr>
                @endforelse
            </tbody>
