...
@section('content')
    ...
    {{-- GRID DE PRODUCTOS (datos reales desde PostgreSQL vía LenteController) --}}
    @if ($lentes->isEmpty())
        <div class="bg-white rounded-xl border border-gray-100 p-10 text-center text-gray-400">
            No hay lentes registrados todavía.
            <a href="{{ route('lentes.create') }}" class="text-emerald-700 font-medium hover:underline">Crea el primero</a>.
        </div>
    @else
            ...
            @foreach ($lentes as $lente)
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition">
                    <div class="relative bg-gray-50 h-40 flex items-center justify-center">
                        @if ($lente->imagen)
                            <img src="{{ asset('storage/' . $lente->imagen) }}" alt="{{ $lente->nombre }}" class="h-28 object-contain">
                        @else
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 text-gray-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <span class="absolute top-3 right-3 text-[11px] font-semibold px-2 py-1 rounded-full
                                     {{ $lente->stock > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600' }}">
                            {{ $lente->stock > 0 ? 'En stock' : 'Agotado' }}
                        </span>
                    </div>

                    <div class="p-4">
                        <h4 class="font-semibold text-gray-900 text-sm">{{ $lente->nombre }}</h4>
                        <p class="text-xs text-gray-400 mt-0.5">{{ $lente->marca->nombre }} · {{ $lente->categoria->nombre }}</p>
                        ...
                        <div class="flex items-center gap-2 mt-4">
                            <a href="{{ route('lentes.show', $lente) }}"
                            ...
                            <a href="{{ route('lentes.edit', $lente) }}"
                            ...
                            <form action="{{ route('lentes.destroy', $lente) }}" method="POST" class="flex-1"
